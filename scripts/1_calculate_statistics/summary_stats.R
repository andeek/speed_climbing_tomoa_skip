...
# Group the data frame by yeasr and calculate the percentage of TRUE tomoa_skip values
...
# Average final times for use of tomoa skip 
m_times_update |>
  filter(rank <=16) |>
  group_by(year) |>
  summarise(
    num_climbers_ts = sum(tomoa_skip),
    num_climbers_no_ts = sum(!tomoa_skip),
    final_avg_with_ts = mean(final[tomoa_skip], na.rm = TRUE)
  )
